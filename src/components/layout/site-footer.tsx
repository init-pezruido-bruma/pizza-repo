"use client";

import Image from "next/image";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { Container } from "@/components/ui/container";
import { siteConfig } from "@/lib/site";
import { cn } from "@/lib/utils";

const mapsHref = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
  siteConfig.mapsQuery,
)}`;

const footerLinks = [
  { href: "/quienes-somos", label: "Acerca" },
  { href: "/contacto#empleos", label: "Empleos" },
  { href: "/terminos", label: "Términos y condiciones" },
  { href: "/aviso-de-privacidad", label: "Política de privacidad" },
  { href: "/facturacion", label: "Facturación" },
] as const;

export function SiteFooter() {
  const pathname = usePathname();
  const clearsAngledCta = pathname === "/quienes-somos";

  return (
    <footer className="bg-gradient-to-r from-[#fcda21] via-[#ef9128] to-[#e13e30] text-white">
      <Container
        className={cn(
          "relative grid gap-10 pb-12 md:grid-cols-[1.15fr_1fr_1.25fr] md:items-start md:gap-8 lg:pb-14",
          clearsAngledCta
            ? "pt-[clamp(4.5rem,8vw,6.5rem)] lg:pt-[clamp(5rem,9vw,7rem)]"
            : "pt-12 lg:pt-14",
        )}
      >
        <div>
          <Image
            src="/images/brand/wordmark-footer.png"
            alt="Incredible Food and Fun"
            width={240}
            height={74}
            className="h-auto w-[200px] drop-shadow-[0_0_12px_rgba(252,218,33,0.65)] sm:w-[220px]"
          />
        </div>

        <nav aria-label="Enlaces del pie">
          <ul className="grid gap-2.5 text-sm font-extrabold uppercase tracking-[0.06em]">
            {footerLinks.map((link) => (
              <li key={link.label}>
                {"external" in link && link.external ? (
                  <a href={link.href} className="transition hover:opacity-80">
                    {link.label}
                  </a>
                ) : (
                  <Link href={link.href} className="transition hover:opacity-80">
                    {link.label}
                  </Link>
                )}
              </li>
            ))}
          </ul>
        </nav>

        <div className="relative space-y-1.5 pt-2 text-sm font-semibold leading-relaxed md:pt-0">
          <p className="pointer-events-none absolute -top-4 right-0 hidden font-display text-3xl font-black leading-none text-[#fcda21] drop-shadow-[0_1px_0_rgba(35,31,32,0.15)] rotate-12 md:block lg:-top-6 lg:text-4xl">
            Since 2007
          </p>
          <p>
            <a
              href={mapsHref}
              target="_blank"
              rel="noopener noreferrer"
              className="transition hover:opacity-80"
            >
              {siteConfig.address.street}
              <br />
              {siteConfig.address.city}, {siteConfig.address.region}{" "}
              {siteConfig.address.postalCode}
            </a>
          </p>
          <p>
            <a href={`tel:${siteConfig.phoneTel}`} className="transition hover:opacity-80">
              {siteConfig.phone}
            </a>
          </p>
          <p>
            <a
              href={`https://wa.me/${siteConfig.whatsapp}`}
              target="_blank"
              rel="noopener noreferrer"
              className="transition hover:opacity-80"
            >
              WhatsApp: {siteConfig.whatsappDisplay}
            </a>
          </p>
          <p>
            <a href={`mailto:${siteConfig.email}`} className="transition hover:opacity-80">
              {siteConfig.email}
            </a>
          </p>
          <div className="flex gap-3 pt-4">
            <SocialCircle href={siteConfig.social.facebook} label="Facebook">
              <FacebookIcon />
            </SocialCircle>
            <SocialCircle href={siteConfig.social.instagram} label="Instagram">
              <InstagramIcon />
            </SocialCircle>
            <SocialCircle href={siteConfig.social.tiktok} label="TikTok">
              <TikTokIcon />
            </SocialCircle>
          </div>
        </div>
      </Container>

      <div className="border-t border-white/35">
        <Container className="flex flex-col gap-2 py-3 text-xs font-semibold sm:flex-row sm:items-center sm:justify-between">
          <p>
            ©{siteConfig.legalName} {new Date().getFullYear()}. Todos los derechos Reservados.
          </p>
          <Link href="/aviso-de-privacidad" className="transition hover:opacity-80">
            Aviso De Privacidad
          </Link>
        </Container>
      </div>
    </footer>
  );
}

function SocialCircle({
  href,
  label,
  children,
}: {
  href: string;
  label: string;
  children: React.ReactNode;
}) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      className="inline-flex size-9 items-center justify-center rounded-full border border-white/80 text-white transition hover:scale-105 hover:bg-white/15 sm:size-10"
    >
      {children}
    </a>
  );
}

function FacebookIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-5" aria-hidden fill="currentColor">
      <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.43c0-3.007 1.792-4.668 4.533-4.668 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073" />
    </svg>
  );
}

function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-5" aria-hidden fill="currentColor">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
    </svg>
  );
}

function TikTokIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-5" aria-hidden fill="currentColor">
      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.95-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
    </svg>
  );
}

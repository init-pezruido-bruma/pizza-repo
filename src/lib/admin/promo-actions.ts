...
import { asActionResult, type ActionResult } from "@/lib/admin/action-result";
import { deletePromoImage, savePromoImage } from "@/lib/storage";

async function requireAdmin() {
  const session = await auth();
  if (!session?.user?.id) throw new Error("Sesión expirada. Vuelve a entrar.");
  return session;
}
...
export async function uploadPromotionAction(formData: FormData): Promise<ActionResult> {
...
export async function publishPromotionAction(formData: FormData): Promise<ActionResult> {
  return asActionResult(async () => {
    await requireAdmin();
...
export async function archivePromotionAction(formData: FormData): Promise<ActionResult> {
  return asActionResult(async () => {
    await requireAdmin();
    const id = String(formData.get("id") || "");
    if (!id) throw new Error("ID requerido");
